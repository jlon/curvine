...
impl DBEngine {
    ...
    /// Reads a batch while preserving one result slot per input key.
    ///
    /// Callers that need to distinguish a missing key from the following key's
    /// value must use this instead of `multi_get_cf`, which intentionally
    /// filters absent values for its existing bulk-read callers.
    pub fn multi_get_cf_optional<'a, K, I>(
        &self,
        cf: &str,
        keys: I,
    ) -> CommonResult<Vec<Option<Vec<u8>>>>
    where
        K: AsRef<[u8]> + 'a,
        I: IntoIterator<Item = &'a K>,
    {
