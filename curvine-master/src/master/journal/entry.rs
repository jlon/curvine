...
pub enum MetadataCommand {
    ...
    ReopenFile(ReopenFileEntry),
    OverWriteFile(OverWriteFileEntry),
    // Committed add/assign needs the full file payload to preserve next_seq.
    AddBlock(CompleteFileEntry),
    CompleteFile(CompleteFileEntry),
    Rename(RenameEntry),
    Mount(MountEntry),
    UnMount(UnMountEntry),
...
impl MetadataCommand {
    pub fn op_id(&self) -> u64 {
        match self {
            ...
            MetadataCommand::OverWriteFile(entry) => entry.op_id,
            MetadataCommand::AddBlock(entry) => entry.op_id,
            MetadataCommand::CompleteFile(entry) => entry.op_id,
            MetadataCommand::Rename(entry) => entry.op_id,
            MetadataCommand::Mount(entry) => entry.op_id,
            MetadataCommand::UnMount(entry) => entry.op_id,
            MetadataCommand::SetAttr(entry) => entry.op_id,
            MetadataCommand::Delete(entry) => entry.op_id,
            MetadataCommand::Free(entry) => entry.op_id,
    ...
    pub fn rpc_id(&self) -> i64 {
        match self {
            ...
            MetadataCommand::OverWriteFile(entry) => entry.rpc_id,
            MetadataCommand::AddBlock(entry) => entry.rpc_id,
            MetadataCommand::CompleteFile(entry) => entry.rpc_id,
            MetadataCommand::Rename(entry) => entry.rpc_id,
            MetadataCommand::Mount(entry) => entry.rpc_id,
            MetadataCommand::UnMount(entry) => entry.rpc_id,
            MetadataCommand::SetAttr(entry) => entry.rpc_id,
            MetadataCommand::Delete(entry) => entry.rpc_id,
            MetadataCommand::Free(entry) => entry.rpc_id,
    ...
    pub fn inode_id(&self) -> Option<i64> {
        match self {
            ...
            MetadataCommand::OverWriteFile(entry) => Some(entry.file.id),
            MetadataCommand::AddBlock(entry) => Some(entry.file.id),
            MetadataCommand::CompleteFile(entry) => Some(entry.file.id),
            MetadataCommand::Rename(_) => None,
            MetadataCommand::Mount(_) => None,
            MetadataCommand::UnMount(_) => None,
            MetadataCommand::SetAttr(_) => None,
            MetadataCommand::Delete(_) => None,
            MetadataCommand::Free(_) => None,
