...
/// Thread-safe generation number using OnceLock (no unsafe needed)
...
pub trait NFSFileSystem: Sync {
    ...
    /// Reads the contents of a file returning (data_slices, EOF)
    /// Returns Vec<DataSlice> to avoid unnecessary data copy.
    ...
    /// Writes the contents of a file returning the number of bytes written.
    /// Takes ownership of data (Vec<u8>) to enable zero-copy optimization.
    /// Note that offset/count may go past the end of the file and that
    ...
    /// Creates a hard link.
    /// Creates a new directory entry (name) in the target directory (dirid)
    /// that refers to the same file as the source file (id).
    /// If not supported due to readonly file system
    /// this should return Err(nfsstat3::NFS3ERR_ROFS)
    async fn link(
        &self,
    ...
    /// Commits (flushes) data written with UNSTABLE mode to stable storage.
    /// Returns the write verifier that should match the one returned by write().
    /// This is called by NFS COMMIT procedure.
    /// Default implementation returns OK immediately (for FILE_SYNC writes).
    ...
    fn fh_to_id(&self, id: &nfs_fh3) -> Result<fileid3, nfsstat3> {
        ...
        // SAFETY: We've verified the length is exactly 16 bytes
