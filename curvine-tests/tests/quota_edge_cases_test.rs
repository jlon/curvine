...
fn quota_edge_cases_test() -> CommonResult<()> {
    ...
    let res: CommonResult<()> = rt.block_on(async move {
        ...
        // 设置配额
        fs.fs_client().add_quota(quota_dir.path(), 10 * 1024).await?; // 10KB
        ...
        if let Some(info) = quota_info {
            assert_eq!(info.used_size, test_data.len() as i64,
                "配额跟踪不正确: expected={}, actual={}", test_data.len(), info.used_size);
        ...
        fs.fs_client().add_quota(quota_root.path(), 10 * 1024 * 1024).await?; // 10MB
        ...
        if let Some(info) = quota_info {
            assert_eq!(info.used_size, deep_data.len() as i64, 
                "深层嵌套目录的配额传播失败: expected={}, actual={}", 
                deep_data.len(), info.used_size);
        ...
        fs.fs_client().add_quota(quota_a.path(), 5 * 1024 * 1024).await?; // 5MB
        fs.fs_client().add_quota(quota_b.path(), 5 * 1024 * 1024).await?; // 5MB
        ...
        assert_eq!(quota_a_info_before.as_ref().unwrap().used_size, file_data.len() as i64);
        ...
        assert_eq!(quota_a_info_after.as_ref().unwrap().used_size, 0, 
            "源配额目录应该减少: expected=0, actual={}", 
            quota_a_info_after.as_ref().unwrap().used_size);
        assert_eq!(quota_b_info_after.as_ref().unwrap().used_size, file_data.len() as i64, 
            "目标配额目录应该增加: expected={}, actual={}", 
            file_data.len(), quota_b_info_after.as_ref().unwrap().used_size);
        ...
        fs.fs_client().add_quota(parent_quota.path(), 10 * 1024 * 1024).await?; // 10MB
        fs.fs_client().add_quota(child_quota.path(), 2 * 1024 * 1024).await?;   // 2MB
        
        // 在子配额目录中创建文件
        let child_file = Path::from_str("/quota_edge_test/parent_quota/child_quota/nested_file.txt")?;
        ...
        assert_eq!(child_info.as_ref().unwrap().used_size, nested_data.len() as i64,
            "子配额目录使用量不正确");
        assert_eq!(parent_info.as_ref().unwrap().used_size, nested_data.len() as i64,
            "父配额目录使用量不正确");
        ...
        fs.fs_client().add_quota(concurrent_quota.path(), 50 * 1024 * 1024).await?; // 50MB
        ...
        for i in 0..100 {
            ...
            let task = async move {
                let file_path = Path::from_str(&format!("/quota_edge_test/concurrent/concurrent_file_{}.txt", i))?;
        ...
        // 验证最终配额使用量的一致性
        let final_quota_info = fs.fs_client().get_quota_info(concurrent_quota.path()).await?;
        if let Some(info) = final_quota_info {
            // 每个文件大约25字节，100个文件约2500字节
            let expected_min = 2000; // 允许一些误差
            let expected_max = 3000;
            
            assert!(info.used_size >= expected_min && info.used_size <= expected_max,
                "并发操作后配额使用量异常: expected=[{}-{}], actual={}",
                expected_min, expected_max, info.used_size);
