...
async fn test_basic_file_operations(fs: &CurvineFileSystem) -> CommonResult<()> {
    ...
    fs.fs_client().add_quota("/quota_test/basic", 1024 * 1024).await?; // 1MB 配额
    ...
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    assert_eq!(quota_info.used_size, expected_size, 
        "配额使用应等于文件大小: expected={}, actual={}", expected_size, quota_info.used_size);
    ...
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    assert_eq!(quota_info.used_size, total_expected_size,
        "追加后配额使用应正确更新: expected={}, actual={}", total_expected_size, quota_info.used_size);
...
async fn test_hardlink_quota_impact(fs: &CurvineFileSystem) -> CommonResult<()> {
    ...
    fs.mkdir(&quota_dir, true).await?;
    fs.fs_client().add_quota("/quota_test/hardlink", 2 * 1024 * 1024).await?; // 2MB 配额
    ...
    // 验证初始配额
    let quota_info = fs.fs_client().get_quota_info("/quota_test/hardlink").await?;
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    ...
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    assert_eq!(quota_info.used_size, file_size * 2, 
        "硬链接创建后配额应增加: expected={}, actual={}", file_size * 2, quota_info.used_size);
    ...
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    assert_eq!(quota_info.used_size, file_size, 
        "硬链接删除后配额应恢复: expected={}, actual={}", file_size, quota_info.used_size);
...
async fn test_directory_rename_quota_propagation(fs: &CurvineFileSystem) -> CommonResult<()> {
    ...
    fs.fs_client().add_quota("/quota_test/rename_src", 1024 * 1024).await?; // 1MB 配额
    fs.fs_client().add_quota("/quota_test/rename_dst", 2 * 1024 * 1024).await?; // 2MB 配额
    ...
    // 验证源目录配额
    let src_quota_info = fs.fs_client().get_quota_info("/quota_test/rename_src").await?;
    ...
    // 验证目标目录配额（应为0）
    let dst_quota_info = fs.fs_client().get_quota_info("/quota_test/rename_dst").await?;
    ...
    assert!(src_quota_info.is_some(), "源配额信息应该存在");
    let src_quota_info = src_quota_info.unwrap();
    assert_eq!(src_quota_info.used_size, 0, 
        "源目录移动后配额应为0: actual={}", src_quota_info.used_size);
    
    let dst_quota_info = fs.fs_client().get_quota_info("/quota_test/rename_dst").await?;
    assert!(dst_quota_info.is_some(), "目标配额信息应该存在");
    let dst_quota_info = dst_quota_info.unwrap();
    assert_eq!(dst_quota_info.used_size, total_size, 
        "目标目录移动后配额应增加: expected={}, actual={}", total_size, dst_quota_info.used_size);
...
async fn test_directory_deletion_quota_cleanup(fs: &CurvineFileSystem) -> CommonResult<()> {
    ...
    fs.mkdir(&quota_dir, true).await?;
    fs.fs_client().add_quota("/quota_test/deletion", 5 * 1024 * 1024).await?; // 5MB 配额
    ...
    // 验证所有文件创建后的配额
    let quota_info = fs.fs_client().get_quota_info("/quota_test/deletion").await?;
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    assert_eq!(quota_info.used_size, total_expected_size, 
        "所有文件创建后配额应正确: expected={}, actual={}", total_expected_size, quota_info.used_size);
    ...
    let quota_info = quota_info.unwrap();
    assert_eq!(quota_info.used_size, file_size, 
        "子目录删除后配额应正确: expected={}, actual={}", file_size, quota_info.used_size);
...
async fn test_file_overwrite_quota_update(fs: &CurvineFileSystem) -> CommonResult<()> {
    ...
    fs.mkdir(&quota_dir, true).await?;
    fs.fs_client().add_quota("/quota_test/overwrite", 1024 * 1024).await?; // 1MB 配额
    ...
    // 验证初始配额
    let quota_info = fs.fs_client().get_quota_info("/quota_test/overwrite").await?;
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    ...
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    assert_eq!(quota_info.used_size, overwrite_size, 
        "覆写后配额应更新: expected={}, actual={}", overwrite_size, quota_info.used_size);
    ...
    assert!(quota_info.is_some(), "配额信息应该存在");
    let quota_info = quota_info.unwrap();
    assert_eq!(quota_info.used_size, smaller_size, 
        "缩小后配额应更新: expected={}, actual={}", smaller_size, quota_info.used_size);
...
async fn test_quota_query_performance(fs: &CurvineFileSystem) -> CommonResult<()> {
    ...
    fs.mkdir(&quota_dir, true).await?;
    fs.fs_client().add_quota("/quota_test/performance", 100 * 1024 * 1024).await?; // 100MB 配额
    ...
    for i in 0..num_subdirs {
        ...
        for j in 0..num_files_per_dir {
            let file_path = Path::from_str(format!("/quota_test/performance/subdir_{}/file_{}.txt", i, j))?;
    ...
    for i in 0..num_queries {
        let quota_info = fs.fs_client().get_quota_info("/quota_test/performance").await?;
        assert!(quota_info.is_some(), "配额信息查询失败 (iteration {})", i);
        let quota_info = quota_info.unwrap();
        assert_eq!(quota_info.used_size, expected_total_size, 
    ...
    for quota in test_quotas {
        info!("Quota: path={}, used={}, limit={}, state={:?}", 
