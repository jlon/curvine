...
fn test_quota_extreme_scale_performance() -> CommonResult<()> {
    ...
    let res: CommonResult<()> = rt.block_on(async move {
        ...
        fs.mkdir(&test_root, true).await?;
        fs.fs_client().add_quota("/extreme_test", 1024i64 * 1024 * 1024 * 100).await?; // 100GB

        let setup_start = Instant::now();
        let mut total_files = 0;
        let mut total_dirs = 0;

        info!("Starting extreme scale setup: {} levels × {} dirs × {} files = {} total files", 
              EXTREME_LEVELS, EXTREME_DIRS_PER_LEVEL, EXTREME_FILES_PER_DIR,
              EXTREME_LEVELS * EXTREME_DIRS_PER_LEVEL * EXTREME_FILES_PER_DIR);

        for level in 0..EXTREME_LEVELS {
            info!("EXTREME: Starting level {} of {} ({}% complete)", 
                  level + 1, EXTREME_LEVELS, 
            ...
            for dir_idx in 0..EXTREME_DIRS_PER_LEVEL {
                ...
                if dir_idx % 50 == 0 {
                    fs.fs_client().add_quota(&dir_path_str, 1024 * 1024 * 10).await?; // 10MB each
                }
                ...
                for file_idx in 0..EXTREME_FILES_PER_DIR {
                    let file_path_str = format!("{}/file_{:04}.txt", dir_path_str, file_idx);
                    let file_path = Path::from_str(&file_path_str)?;
                    let content = format!("Extreme scale test file {} in level {} dir {}", file_idx, level, dir_idx);
                    fs.write_string(&file_path, &content).await?;
                    total_files += 1;
                }
                ...
                if total_dirs % 500 == 0 {
                    ...
                    info!("EXTREME: {} dirs, {} files ({:.1}% complete, {:.0} files/sec)", 
                          total_dirs, total_files,
                          (total_files as f64 / (EXTREME_LEVELS * EXTREME_DIRS_PER_LEVEL * EXTREME_FILES_PER_DIR) as f64) * 100.0,
                          rate);
                }
            }
        }
        ...
        info!("✓ EXTREME setup completed: {} files in {:?} ({:.0} files/sec)", 
              total_files, setup_duration, total_files as f64 / setup_duration.as_secs_f64());
        ...
        info!("✓ EXTREME performance: {} queries with {} files in {:?}, avg: {:?}", 
        ...
        assert!(avg_extreme_time < Duration::from_millis(1), 
                "EXTREME SCALE FAILED: average query time {:?} exceeds 1ms with {} files", 
                avg_extreme_time, total_files);
        ...
        info!("  Files processed: {} ({:.1}M files)", total_files, total_files as f64 / 1_000_000.0);
        info!("  O(1) performance maintained: {:?} avg ({:.0} queries/sec)", avg_extreme_time, extreme_qps);
        info!("  Setup: {:?} ({:.0} files/sec)", setup_duration, total_files as f64 / setup_duration.as_secs_f64());
        info!("  Cleanup: {:?} ({:.0} files/sec)", cleanup_duration, total_files as f64 / cleanup_duration.as_secs_f64());
